...
- [x] **Recipe file v2 — foundation shipped.** `engine/recipe.js`: versioned format (`recipeVersion: 2`) wrapping the book config with a `seed` (reproduces page *structure* — ordering/difficulty) and a `pageState[]` per-page layer (overrides today, reserved Fabric `canvasState` for the editor). v1 recipes migrate automatically on load; Book Builder saves/loads v2; per-page border override honored at render. *Remaining for full reproduction: seedable generators so exact puzzle grids reproduce (currently structure reproduces, grid content re-rolls) — needed before the page editor's per-page reroll.*
